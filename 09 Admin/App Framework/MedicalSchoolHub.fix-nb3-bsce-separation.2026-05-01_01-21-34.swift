...
struct CourseTrackerPage: View {
    ...
    var currentItems: [CourseTrackerItem] {
        let direct = loadCourseTracker(trackerFileForSelectedLeaf())
        ...
        if selectedLeaf == "T2 • NB3" {
            let rootNB3 = loadCourseTracker("\(base)/09 Admin/App Data/Course Trackers/nb3_tracker.csv")
            if !rootNB3.isEmpty { return rootNB3 }
            return items
        }

        if selectedLeaf == "T2 • BSCE" {
            let rootBSCE = loadCourseTracker("\(base)/09 Admin/App Data/Course Trackers/bsce_t2_tracker.csv")
            if !rootBSCE.isEmpty { return rootBSCE }
            return bsceItems
        }
