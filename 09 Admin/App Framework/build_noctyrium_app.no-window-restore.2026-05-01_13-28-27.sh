...
sleep 0.4
echo "Purging Noctyrium saved window state..."
rm -rf "$HOME/Library/Saved Application State/com.jd.noctyrium.savedState" 2>/dev/null || true
rm -rf "$HOME/Library/Saved Application State/Noctyrium.savedState" 2>/dev/null || true
defaults write com.jd.noctyrium NSQuitAlwaysKeepsWindows -bool false >/dev/null 2>&1 || true
defaults write com.jd.noctyrium ApplePersistenceIgnoreState -bool true >/dev/null 2>&1 || true
defaults write com.apple.systempreferences NSQuitAlwaysKeepsWindows -bool false >/dev/null 2>&1 || true
...
echo "Final saved-state purge before launch..."
rm -rf "$HOME/Library/Saved Application State/com.jd.noctyrium.savedState" 2>/dev/null || true
rm -rf "$HOME/Library/Saved Application State/Noctyrium.savedState" 2>/dev/null || true
open -F -n "$DEST_APP"
