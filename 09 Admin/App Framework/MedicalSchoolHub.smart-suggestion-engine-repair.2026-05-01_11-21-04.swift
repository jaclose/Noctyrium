...
func buildSmartSuggestions(tasks: [TaskItem], journal: [JournalEntry], nb3Items: [CourseTrackerItem], bsceItems: [CourseTrackerItem]) -> [SmartSuggestion] {
    ...
    let openTasks = tasks.filter { $0.status.lowercased() != "done" && $0.completed.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
    let taskText = openTasks.map { $0.title.lowercased() }.joined(separator: " | ")
    let recentJournal = journal.suffix(7).map {
        "\($0.today) \($0.tomorrow) \($0.blockers)"
    }.joined(separator: " ").lowercased()

    // 1. Tasks first — the written command always outranks the algorithm.
    for task in openTasks.prefix(6) {
        let t = task.title.lowercased()

        if t.contains("bsce") || t.contains("ftm") || t.contains("msk") || t.contains("cpr") || t.contains("er") || t.contains("dm") || t.contains("nb1") || t.contains("nb2") {
            add("Task-driven: \(task.title) → treat as BSCE / non-current-module priority.")
        } else if t.contains("lecture") || t.contains("lectures") || t.range(of: #"l\d+"#, options: .regularExpression) != nil || t.range(of: #"nb\s*\d+"#, options: .regularExpression) != nil {
            add("Task-driven: \(task.title) → map this to NB3 lecture execution.")
        } else if t.contains("dla") {
            add("Task-driven: \(task.title) → clear the matching NB3 DLA before second passes.")
        } else if t.contains("question") || t.contains("pq") || t.contains("imcq") || t.contains("esoft") {
            add("Task-driven: \(task.title) → complete question work, then review explanations.")
        } else {
            add("Task-driven: \(task.title)")
        }
    }

    // 2. NB3 sweep — first passes before second passes.
    let nb3Core = nb3.filter { ["lecture", "dla"].contains($0.type.lowercased()) }
    let nb3NoPass = nb3Core.filter { $0.quality <= 0 }
    let nb3OnePass = nb3Core.filter { $0.quality == 1 }
    let nb3SinglePassCount = nb3Core.filter { $0.quality >= 1 }.count

    if let firstLecture = nb3NoPass.first(where: { $0.type.lowercased() == "lecture" }) {
        add("NB3 priority: first pass \(firstLecture.label). No second-pass luxury until the untouched lectures are moving.")
    }

    if let firstDLA = nb3NoPass.first(where: { $0.type.lowercased() == "dla" }) {
        add("NB3 DLA: complete first pass on \(firstDLA.label). DLAs are high-yield scaffolding.")
    }

    if nb3NoPass.isEmpty || nb3SinglePassCount >= 10 {
        if let weak = nb3OnePass.first {
            add("NB3 consolidation: push \(weak.label) from red to young with a second pass.")
        }
    }

    let nb3PQ = nb3.filter { $0.type.lowercased() == "pq" && $0.quality <= 0 }
    if nb3NoPass.count <= 3, let pq = nb3PQ.first {
        add("NB3 question pressure: start \(pq.label), then log score and explanations.")
    }

    // 3. BSCE sweep — PLG first, then correlates/pathways, then IMCQ/eSoft.
    let bscePLG = bsce.filter { $0.type.lowercased() == "plg" && $0.quality <= 0 }
    let bsceCorrelates = bsce.filter {
        ["correlates", "pathways", "weak areas", "term review"].contains($0.type.lowercased()) && $0.quality <= 0
    }
    let bsceQuestions = bsce.filter {
        ["imcq", "esoft", "pq"].contains($0.type.lowercased()) && $0.quality <= 0
    }

    if !taskText.contains("bsce") && !recentJournal.contains("bsce") {
        if let plg = bscePLG.first {
            add("BSCE foundation: do \(plg.label). PLG comes before random question grinding.")
        }

        if let corr = bsceCorrelates.first {
            add("BSCE review: sweep \(corr.label) with one-pagers / summaries before more scattered testing.")
        }

        if bscePLG.count <= 2, let q = bsceQuestions.first {
            add("BSCE questions: complete \(q.label), then review every explanation.")
        }
    }

    // 4. Journal-aware nudge.
    if recentJournal.contains("burnout") || recentJournal.contains("tired") || recentJournal.contains("fatigue") {
        add("Energy-aware adjustment: use a short closed-loop block — 25 min lecture/DLA + 10 min recall. No heroic fog-studying.")
    }

    if output.isEmpty {
        add("Maintenance: no urgent weak point detected. Do one NB3 recall pass, then one BSCE PLG/correlates sweep.")
    }
