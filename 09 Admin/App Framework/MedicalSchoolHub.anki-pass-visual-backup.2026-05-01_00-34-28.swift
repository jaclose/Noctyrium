...
struct HubView: View {
    ...
    func updateTrackerItem(_ item: CourseTrackerItem, _ action: String) {
        ...
        let minutes = item.type.lowercased() == "lecture" ? 35 : item.type.lowercased() == "dla" ? 25 : 15
        let eventType = item.type.lowercased() == "lecture" ? "Lecture" : item.type.lowercased() == "dla" ? "DLA" : "Practice"
        log(type: eventType, minutes: minutes, cards: 0, note: "\(item.label) → \(newStatus) / \(newQuality) passes")
    }
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            HStack(spacing: 5) {
                ...
                SmallTrackerButton("Reset") { updateItem(item, "reset") }
            }
...
struct SmallTrackerButton: View {
    ...
    var body: some View {
        Button(title, action: action)
            ...
            .frame(width: title == "Mature" ? 52 : title == "Reset" ? 42 : 34, height: 25)
            .background(
