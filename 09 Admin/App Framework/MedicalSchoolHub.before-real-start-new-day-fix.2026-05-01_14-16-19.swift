...
func startNewStudyDay() {
        ...
        if !FileManager.default.fileExists(atPath: archivePath) {
            let header = "date_key,study_minutes,anki_cards,completed_tasks,open_tasks,standup_today,standup_tomorrow,standup_blockers,energy,rating,updated\n"
            try? header.write(toFile: archivePath, atomically: true, encoding: .utf8)
        }
        ...
        let studyMinutes = max(0, stats.todayMinutes + stats.manualMinutesToday)
        let ankiCards = max(0, stats.todayCards)
        ...
        append(archiveLine, to: archivePath)

        try? nextKey.write(toFile: activeStudyDayFile, atomically: true, encoding: .utf8)
        try? nextKey.write(toFile: manualStudyDayFile, atomically: true, encoding: .utf8)

        append("\(stamp),System,0,0,Start New Study Day archived \(currentKey) and opened \(nextKey) | study_day=\(nextKey)\n", to: logFile)
        ...
        status = "Started study day \(nextKey). Archived \(currentKey)."
        refresh()
        runSuggestionSweep()
    }
