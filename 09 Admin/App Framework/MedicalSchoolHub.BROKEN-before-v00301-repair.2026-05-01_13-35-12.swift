...
var body: some View {
    ...
    func startNewStudyDay() {
        ...
        let adjusted = calendar.date(byAdding: .hour, value: -4, to: now) ?? now
        ...
        let studyDay = dayFormatter.string(from: adjusted)
        ...
        let archiveDir = logURL.deletingLastPathComponent().appendingPathComponent("Archived Study Logs", isDirectory: true)

        try? fm.createDirectory(at: archiveDir, withIntermediateDirectories: true)

        if fm.fileExists(atPath: logFile) {
            let archiveURL = archiveDir.appendingPathComponent("study_log_backup_\(stamp).csv")
            try? fm.copyItem(at: logURL, to: archiveURL)
        }
    func addAnki20() {
        log(type: "Anki Correction", minutes: 0, cards: 20, note: "Manual correction: +20 Anki cards")
        refresh()
        runSuggestionSweep()
    }

    func addStudy25() {
        log(type: "Study Correction", minutes: 25, cards: 0, note: "Manual correction: +25 minutes")
        refresh()
        runSuggestionSweep()
    }
        ...
        let freshCSV = """
date,type,minutes,cards,note
\(isoStamp),System,0,0,Start New Study Day → \(studyDay)

"""
        try? freshCSV.write(toFile: logFile, atomically: true, encoding: .utf8)
                ...
                GlassCard {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        HStack(spacing: 10) {
                            ...
                            Button("−20 Anki Cards", action: subtractAnki20)
                    Button("Add +20 Anki Cards", action: addAnki20)
                    Button("−30 Minutes", action: subtractStudy30)
                    Button("Add +25 Minutes", action: addStudy25) {
                                subtractAnki20()
                            }
                            .buttonStyle(.plain)
                            .font(.system(size: 12, weight: .black, design: .rounded))
