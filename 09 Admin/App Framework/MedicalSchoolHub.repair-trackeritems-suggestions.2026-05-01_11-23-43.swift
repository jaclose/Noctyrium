...
struct HubView: View {
    ...
    @State private var nb3Items: [CourseTrackerItem] = []
    ...
    func refresh() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            DispatchQueue.main.async {
                ...
                bsceItems = bsce
                smartSuggestions = buildSmartSuggestions(tasks: t, journal: j, nb3Items: tracker, bsceItems: bsce)
                )
                status = output
                refreshing = false
            }
    ...
    func runSuggestionSweep() {
        ...
        let t = loadTasks(taskFile)
        let j = loadJournal(journalFile)
        let nb3 = loadCourseTracker(nb3TrackerFile)
        ...
        let reportPath = "\(base)/09 Admin/Reports/noctyrium_suggestion_sweep_\(DateFormatter.fileSafe.string(from: Date())).md"

        let body = smartSuggestions
            .sorted { $0.priority > $1.priority }
            .map { "- [\($0.source)] \($0.title) — \($0.reason)" }
            .joined(separator: "\n")

        let report = """
        # Noctyrium Suggestion Sweep

        Generated: \(Date())

        ## Suggestions

        \(body.isEmpty ? "No active suggestions." : body)
        """

        try? report.write(toFile: reportPath, atomically: true, encoding: .utf8)
        status = "Suggestion sweep complete: \(reportPath)"
    }
...
struct DashboardPage: View {
    ...
    let nb3Items: [CourseTrackerItem]
...
func buildSmartSuggestions(tasks: [TaskItem], journal: [JournalEntry], nb3Items: [CourseTrackerItem], bsceItems: [CourseTrackerItem]) -> [SmartSuggestion] {
    ...
    // 1. Tasks first. Direct user intent beats algorithmic cleverness.
    for task in openTasks.prefix(10) {
        ...
        if lower.contains("bsce") ||
            ...
            add(
                ...
                "This task mentions BSCE or a non-current module, so it should be pulled into the BSCE/module prep lane.",
                "Tasks",
                110,
                "purple"
            )
            ...
            add(
                ...
                "This task looks like current NB3 lecture/course work. Treat it as current-module priority.",
                "Tasks",
                105,
                "cyan"
            )
            ...
            add(
                ...
                "Open task still exists. Clear it, convert it, or intentionally defer it before making a new plan.",
                "Tasks",
                78,
                "blue"
            )
    ...
    // 2. Journal scan. Newest entries are assumed to be loaded first.
    for entry in journal.prefix(5) {
        ...
        if combined.contains("bsce") {
            add(
                ...
                "Recent journal context mentions BSCE. Keep that thread alive instead of context-switching blindly.",
                "Journal",
                90,
                "purple"
            )
        ...
        if combined.contains("nb3") || combined.contains("lecture") || combined.contains("dla") {
            add(
                "Journal carryover: NB3 recovery",
                "Recent journal context mentions NB3, lectures, or DLAs. Convert reflection into one concrete pass.",
                "Journal",
                88,
                "cyan"
            )
        ...
        if combined.contains("behind") || combined.contains("block") || combined.contains("fatigue") || combined.contains("burnout") {
            add(
                "Stabilize: one core pass + one small question block",
                "Journal shows friction. The smart move is narrow execution, not a heroic fake plan.",
                "Journal",
                86,
                "orange"
            )
    ...
    // 3. NB3 sweep.
    let nb3Core = nb3Items.filter {
        ["lecture", "dla"].contains($0.type.lowercased())
    }

    let nb3FirstPassCount = nb3Core.filter { $0.quality >= 1 }.count
    let nb3ZeroPass = nb3Core.filter { $0.quality < 1 }

    if let firstZero = nb3ZeroPass.first {
        add(
            "NB3 first pass: \(firstZero.label)",
            "This lecture/DLA has zero passes. First-pass coverage outranks second-pass polishing until the module has enough coverage.",
            "NB3 Tracker",
            100,
            "red"
        )
    ...
    if let zeroDLA = nb3Items.first(where: { $0.type.lowercased() == "dla" && $0.quality < 1 }) {
        add(
            "DLA catch-up: \(zeroDLA.label)",
            "DLAs carry high module weight and should not lag behind lectures.",
            "NB3 Tracker",
            96,
            "orange"
        )
    ...
    if nb3FirstPassCount >= 10 {
        if let secondPassTarget = nb3Core.first(where: { $0.quality == 1 }) {
            add(
                "NB3 second pass: \(secondPassTarget.label)",
                "You have enough first-pass coverage to begin upgrading weak material from red to young.",
                "NB3 Tracker",
                84,
                "green"
            )
    ...
    if let pq = nb3Items.first(where: { ["pq", "esoft", "imcq"].contains($0.type.lowercased()) && $0.quality < 1 }) {
        add(
            "NB3 question pressure: \(pq.label)",
            "Questions become more valuable once first-pass coverage is moving.",
            "NB3 Tracker",
            72,
            "blue"
        )
    ...
    // 4. BSCE sweep.
    let bscePLGs = bsceItems.filter { $0.type.lowercased() == "plg" }
    let bsceCorrelates = bsceItems.filter {
        ["correlates", "pathways", "weak areas", "module"].contains($0.type.lowercased())
    }
    let bsceQuestions = bsceItems.filter {
        ["esoft", "imcq"].contains($0.type.lowercased())
    }

    if let plg = bscePLGs.first(where: { $0.quality < 1 }) {
        add(
            "BSCE PLG: \(plg.label)",
            "PLG review is the front door for BSCE prep. Do this before scattered question review.",
            "BSCE Tracker",
            98,
            "purple"
        )
    ...
    if let correlate = bsceCorrelates.first(where: { $0.quality < 1 }) {
        add(
            "BSCE correlates/pathways: \(correlate.label)",
            "Correlates and pathways turn module memory into exam-usable structure.",
            "BSCE Tracker",
            82,
            "indigo"
        )
    ...
    if let questionSet = bsceQuestions.first(where: { $0.quality < 1 }) {
        add(
            "BSCE question set: \(questionSet.label)",
    ...
    // 5. Boost tracker suggestions if task list already points there.
    if taskText.contains("bsce") {
        result = result.map {
            if $0.source == "BSCE Tracker" {
                return SmartSuggestion(
                    title: $0.title,
                    reason: $0.reason + " Your task list already mentions BSCE, so this is paired with the task queue.",
                    source: $0.source,
    ...
    if taskText.contains("nb3") || taskText.contains("lecture") || taskText.contains("dla") {
        result = result.map {
            if $0.source == "NB3 Tracker" {
                return SmartSuggestion(
                    title: $0.title,
                    reason: $0.reason + " Your task list already points toward NB3/current module work.",
                    source: $0.source,
                    priority: $0.priority + 6,
                    color: $0.color
                )
