...
struct CourseTrackerPage: View {
    ...
    let base: String
    let open: (String) -> Void
    let updateItem: (String, Int?, String?, String?, String?) -> Void
    let addItem: (String, String, String, String) -> Void

    @State private var selectedCourse = "01 BPM 501"
    @State private var selectedModule = "NB3"
    @State private var newPracticeType = "PLG"
    @State private var newPracticeText = ""
    @State private var newPracticeNotes = ""

    let practiceTypes = ["PLG", "PQ", "Esoft", "IMCQ", "Small Group", "Other Event"]

    var lectureItems: [CourseTrackerItem] {
        items.filter { ["Lecture", "DLA"].contains($0.type) }
    }

    var practiceItems: [CourseTrackerItem] {
        items.filter { ["PQ", "PLG", "Esoft", "IMCQ", "Small Group", "Other Event"].contains($0.type) }
    }

    var futureItems: [CourseTrackerItem] {
        items.filter { !["Lecture", "DLA", "PQ", "PLG", "Esoft", "IMCQ", "Small Group", "Other Event"].contains($0.type) }
    }

    var masteredCount: Int {
        items.filter { $0.status == "mastered" || $0.color.lowercased() == "blue" }.count
    }

    var needsWorkCount: Int {
        items.filter { $0.status == "needs_work" || $0.color.lowercased() == "red" }.count
    }

    var completedCount: Int {
        items.filter { ["completed", "mastered"].contains($0.status) || ["green", "blue"].contains($0.color.lowercased()) }.count
    }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        HStack(alignment: .top) {
                            VStack(alignment: .leading, spacing: 6) {
                                ...
                                Text("Dense pass ledger for lectures, DLAs, weekly practice, PLGs, eSofts, and IMCQ reviews.")
                            ...
                            VStack(alignment: .trailing, spacing: 8) {
                                Text("\(selectedCourse) • \(selectedModule)")
                                    .font(.system(size: 14, weight: .semibold, design: .rounded))
                                    .padding(.horizontal, 12)
                                    .padding(.vertical, 7)
                                    .background(.white.opacity(0.10), in: Capsule())

                                MiniButton("Open NB3 Folder") {
                                    open("\(base)/10 Courses/Term 1/01 BPM 501/NB 3")
                                }

                                MiniButton("Open Tracker CSV") {
                                    open("\(base)/09 Admin/App Data/Course Trackers/nb3_tracker.csv")
                                }
                            }
                        }
                        ...
                        HStack(spacing: 10) {
                            Picker("Course", selection: $selectedCourse) {
                                Text("01 BPM 501").tag("01 BPM 501")
                                Text("01 BPM 500").tag("01 BPM 500")
                                Text("02 PPM 500").tag("02 PPM 500")
                                Text("02 PPM 501").tag("02 PPM 501")
                                Text("02 PPM 502").tag("02 PPM 502")
                            }
                            .pickerStyle(.menu)
                ...
                LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 12), count: 5), spacing: 12) {
                    TrackerHero(title: "Total", value: "\(items.count)", symbol: "list.bullet.rectangle")
                    TrackerHero(title: "Completed", value: "\(completedCount)", symbol: "checkmark.seal")
                    TrackerHero(title: "Mastered", value: "\(masteredCount)", symbol: "crown")
                    TrackerHero(title: "Needs Work", value: "\(needsWorkCount)", symbol: "flame")
                    TrackerHero(title: "Practice", value: "\(practiceItems.count)", symbol: "questionmark.bubble")
                }

                GlassCard {
                    VStack(alignment: .leading, spacing: 12) {
                        PanelHeader("Lecture + DLA Pass Ledger", "Small, dense, readable. The workbench, not the museum.")

                        HStack {
                            Text("Item").frame(width: 64, alignment: .leading)
                            Text("Type").frame(width: 58, alignment: .leading)
                            Text("Title").frame(maxWidth: .infinity, alignment: .leading)
                            Text("Passes").frame(width: 150, alignment: .leading)
                        ...
                        ForEach(groupedWeeks(lectureItems), id: \.0) { week, weekItems in
                            VStack(alignment: .leading, spacing: 6) {
                                Text(week)
                                ...
                                ForEach(weekItems) { item in
                                    TrackerLedgerRow(item: item, updateItem: updateItem)
                                }
                            }
                        }
                    }
                ...
                GlassCard {
                    VStack(alignment: .leading, spacing: 12) {
                        PanelHeader("Weekly Practice Layer", "PQs, PLGs, eSofts, IMCQ reviews, and small-group framework.")

                        HStack(spacing: 10) {
                            Picker("Type", selection: $newPracticeType) {
                            ...
                            TextField("Add PLG / PQ / eSoft / IMCQ review...", text: $newPracticeText)
                                .textFieldStyle(.plain)
                                .padding(10)
                                .background(.white.opacity(0.08), in: RoundedRectangle(cornerRadius: 12))

                            TextField("Notes / weak area", text: $newPracticeNotes)
                                .textFieldStyle(.plain)
                                .padding(10)
                                .background(.white.opacity(0.08), in: RoundedRectangle(cornerRadius: 12))

                            Button("Add") {
                                addItem(newPracticeType, newPracticeText, "Custom", newPracticeNotes)
                                newPracticeText = ""
                                newPracticeNotes = ""
                            }
                            .buttonStyle(.plain)
                        ...
                        ForEach(groupedWeeks(practiceItems), id: \.0) { week, weekItems in
                            VStack(alignment: .leading, spacing: 6) {
                                Text(week)
                                    .font(.system(size: 13, weight: .bold, design: .rounded))
                                    .foregroundStyle(.white.opacity(0.82))
                ...
                GlassCard {
                    VStack(alignment: .leading, spacing: 10) {
                        PanelHeader("Later Framework", "Small groups, OSPEs, quiz reviews, calendar hooks, and Anki sync can plug in here.")
                        Text("Current focus stays clean: Lectures, DLAs, PQs, PLGs, eSofts, and IMCQ. The rest is staged without clutter.")
                            .font(.system(size: 13))
                            .foregroundStyle(.secondary)
                    }
                }
    ...
    func groupedWeeks(_ source: [CourseTrackerItem]) -> [(String, [CourseTrackerItem])] {
        let grouped = Dictionary(grouping: source) { $0.week }
...
struct TrackerLedgerRow: View {
    let item: CourseTrackerItem
    let updateItem: (String, Int?, String?, String?, String?) -> Void

    var passCount: Int {
        max(0, min(4, item.quality))
    }

    var qualityLabel: String {
        switch item.color.lowercased() {
        case "red": return "Needs Work"
        case "orange": return "Shaky"
        case "green": return "Good"
        case "blue": return "Mastered"
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            HStack(spacing: 4) {
                Button("−") {
                    updateItem(item.id, -1, nil, nil, nil)
                }
                .buttonStyle(.plain)
                ...
                ForEach(1...4, id: \.self) { n in
                    Circle()
                        .fill(n <= passCount ? qualityColor.opacity(0.95) : Color.white.opacity(0.12))
            ...
            Menu {
                Button("Red — Needs Work") {
                    updateItem(item.id, nil, "needs_work", "red", nil)
                }
                Button("Orange — Shaky") {
                    updateItem(item.id, nil, "in_progress", "orange", nil)
                }
                Button("Green — Completed") {
                    updateItem(item.id, nil, "completed", "green", nil)
                }
                Button("Blue — Mastered") {
                    updateItem(item.id, nil, "mastered", "blue", nil)
                }
                Button("Gray — Reset") {
                    updateItem(item.id, nil, "not_started", "gray", nil)
                }
            } label: {
                Text(qualityLabel)
                    .font(.system(size: 11, weight: .bold, design: .rounded))
    ...
    func shortID(_ id: String) -> String {
        id.replacingOccurrences(of: "PQ", with: "PQ ")
          .replacingOccurrences(of: "DLA", with: "DLA ")
    }
    ...
    func typeColor(_ type: String) -> Color {
        ...
        case "DLA": return .purple
        case "PQ": return .green
        case "PLG": return .orange
        case "Esoft": return .yellow
        case "IMCQ": return .pink
