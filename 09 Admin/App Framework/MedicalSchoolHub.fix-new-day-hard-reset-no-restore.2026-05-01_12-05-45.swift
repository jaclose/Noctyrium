...
var body: some View {
    ...
    func startNewStudyDay() {
        ...
        let cutoffAdjustedNow = calendar.date(byAdding: .hour, value: -4, to: now) ?? now

        let dayFormatter = DateFormatter()
        dayFormatter.locale = Locale(identifier: "en_US_POSIX")
        dayFormatter.dateFormat = "yyyy-MM-dd"

        let currentStudyDay = dayFormatter.string(from: cutoffAdjustedNow)

        func studyDayKey(from rawDate: String) -> String? {
            let trimmed = rawDate.trimmingCharacters(in: .whitespacesAndNewlines)
            if trimmed.isEmpty { return nil }

            let iso = ISO8601DateFormatter()
            if let parsed = iso.date(from: trimmed) {
                let adjusted = calendar.date(byAdding: .hour, value: -4, to: parsed) ?? parsed
                return dayFormatter.string(from: adjusted)
            }
        ...
        if let raw = try? String(contentsOfFile: logFile) {
            ...
            for line in lines.dropFirst() {
                ...
                let date = p[0]
                let type = p[1].lowercased()
                let minutes = Int(p[2].trimmingCharacters(in: .whitespacesAndNewlines)) ?? 0
                let cards = Int(p[3].trimmingCharacters(in: .whitespacesAndNewlines)) ?? 0
                let note = p[4].lowercased()

                guard studyDayKey(from: date) == currentStudyDay else { continue }

                // Avoid double-counting prior reset markers as "real work."
                if type == "system" && note.contains("start new study day") {
                    continue
                }
        ...
        if minutesToday != 0 {
            append("\(stamp),Study,\(-minutesToday),0,Start New Study Day reset: zeroed study minutes from \(minutesToday)\n", to: logFile)
        }

        if cardsToday != 0 {
            append("\(stamp),Anki,0,\(-cardsToday),Start New Study Day reset: zeroed Anki cards from \(cardsToday)\n", to: logFile)
        }

        append("\(stamp),System,0,0,Start New Study Day → \(currentStudyDay)\n", to: logFile)
