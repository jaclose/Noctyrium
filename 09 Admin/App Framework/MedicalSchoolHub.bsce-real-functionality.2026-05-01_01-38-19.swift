...
struct TrackerDenseRow: View {
    ...
    var tickStrip: some View {
        HStack(spacing: 3) {
            ForEach(0..<maxTicks, id: \.self) { i in
                Image(systemName: i < min(item.quality, maxTicks) ? "checkmark.circle.fill" : "circle")
                    .font(.system(size: 10, weight: .bold))
                    .foregroundColor(i < min(item.quality, maxTicks) ? mainColor : .white.opacity(0.25))
            }
        }
    }
    ...
    var actionButtons: some View {
        HStack(spacing: 5) {
            if isCompletionOnly {
                ...
                TrackerMiniAction("Done") { updater(item, "complete") }
                TrackerMiniAction("Reset") { updater(item, "reset") }
            } else if isLimitedPassNoAnki {
    ...
    var isBSCE: Bool {
        item.id.uppercased().hasPrefix("BSCE_") || item.week.uppercased().contains("BSCE")
    }

    var isCompletionOnly: Bool {
        isBSCE && ["esoft", "imcq", "pq"].contains(type)
    }

    var isLimitedPassNoAnki: Bool {
        isBSCE && type == "plg"
    }

    var allowsAnki: Bool {
        if !isBSCE { return ["lecture", "dla", "correlates", "pathways", "weak areas", "module"].contains(type) }
        return ["module", "correlates", "pathways", "weak areas", "term review"].contains(type)
    }

    var maxTicks: Int {
        if isCompletionOnly { return 1 }
        if isLimitedPassNoAnki { return 2 }
        return 3
    }

    var actionWidth: CGFloat {
        if isCompletionOnly { return 172 }
        if isLimitedPassNoAnki { return 235 }
        return 292
    }
    ...
    var statusLabel: String {
        if item.status.lowercased() == "working" { return "working on it" }
        if isCompletionOnly {
            return item.quality > 0 ? "completed" : "not completed"
        }
        if item.quality <= 0 { return "not started" }
        if item.quality == 1 { return isLimitedPassNoAnki ? "first pass" : "needs mastery" }
        if item.quality == 2 { return "young / complete" }
        return "mature"
    }
}
