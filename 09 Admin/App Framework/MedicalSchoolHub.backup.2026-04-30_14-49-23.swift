...
struct PromptPage: View {
    ...
    var body: some View {
        LazyVGrid(columns: [GridItem(.adaptive(minimum: 220), spacing: 16)], spacing: 16) {
            ActionTile("Prompt Library", "All prompts", "text.book.closed") { open("\(base)/10 Prompt Library") }
            ActionTile("Anki Prompts", "text/file to cards", "sparkles.rectangle.stack") { open("\(base)/10 Prompt Library/Anki Card Generation") }
            ActionTile("CSS / HTML", "card templates", "curlybraces.square") { open("\(base)/10 Prompt Library/Anki CSS and HTML") }
            ActionTile("Uploads", "future file workflow", "doc.badge.plus") { open("\(base)/10 Prompt Library/File Upload Workflows") }
        }
    }
}
