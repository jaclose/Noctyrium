...
var body: some View {
    ...
    func subtractAnki20() {
        log(type: "Anki Correction", minutes: 0, cards: -20, note: "Manual correction: subtract 20 Anki cards")
        refresh()
        runSuggestionSweep()
    }

    func subtractStudy30() {
        log(type: "Study Correction", minutes: -30, cards: 0, note: "Manual correction: subtract 30 study minutes")
        refresh()
        runSuggestionSweep()
    }
...
func startNewStudyDay() {
        let currentKey = productivityDayKey
        let nextKey = noctyriumNextDateKey(after: currentKey)

        let archivePath = "\(base)/09 Admin/App Data/Noctyrium/Productivity/productivity_day_archive.csv"
        ...
        func appendArchiveLine(_ line: String) {
        ...
        let archiveLine = [
            ...
            csv(standupEnergy),
            csv(standupRating),
            csv(now.string(from: Date()))
        ].joined(separator: ",") + "\n"

        appendArchiveLine(archiveLine)
        ...
        if studyMinutes > 0 {
            appendLog("\(stamp),Study,\(-studyMinutes),0,Start New Study Day reset study counter for \(nextKey)\n")
        }

        if ankiCards > 0 {
            appendLog("\(stamp),Anki,0,\(-ankiCards),Start New Study Day reset Anki counter for \(nextKey)\n")
        }

        appendLog("\(stamp),System,0,0,Start New Day archived \(currentKey) and opened \(nextKey)\n")
