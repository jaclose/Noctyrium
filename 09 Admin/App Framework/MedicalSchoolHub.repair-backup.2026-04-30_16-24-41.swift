...
struct CourseTrackerPage: View {
    ...
    var focusItems: [CourseTrackerItem] {
        items.filter { ["Lecture", "DLA", "PQ"].contains($0.type) }
    }

    var lectures: [CourseTrackerItem] { focusItems.filter { $0.type == "Lecture" } }
    var dlas: [CourseTrackerItem] { focusItems.filter { $0.type == "DLA" } }
    var pqs: [CourseTrackerItem] { focusItems.filter { $0.type == "PQ" } }

    var weakCount: Int {
        focusItems.filter { ["red", "orange"].contains($0.color.lowercased()) }.count
    }

    var masteredCount: Int {
        focusItems.filter { $0.color.lowercased() == "blue" }.count
    }
            ...
            VStack(alignment: .leading, spacing: 22) {
                HStack(alignment: .top) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text("NB3 lecture, DLA, and practice question command map.")
                ...
                LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 14), count: 5), spacing: 14) {
                    TrackerHero(title: "Lectures", value: "\(completed(lectures))/\(lectures.count)", symbol: "play.rectangle")
                    TrackerHero(title: "DLA", value: "\(completed(dlas))/\(dlas.count)", symbol: "doc.text")
                    TrackerHero(title: "PQ", value: "\(completed(pqs))/\(pqs.count)", symbol: "questionmark.circle")
                    TrackerHero(title: "Weak", value: "\(weakCount)", symbol: "exclamationmark.triangle")
                    TrackerHero(title: "Mastered", value: "\(masteredCount)", symbol: "crown")
                }

                GlassCard {
                    VStack(alignment: .leading, spacing: 14) {
                        PanelHeader("NB3 Readiness", "Color tells the truth faster than a spreadsheet.")

                        TrackerProgressBar(title: "Lecture Pass", done: completed(lectures), total: max(lectures.count, 1))
                        TrackerProgressBar(title: "DLA Pass", done: completed(dlas), total: max(dlas.count, 1))
                        TrackerProgressBar(title: "PQ Pass", done: completed(pqs), total: max(pqs.count, 1))

                        Text("Color code: gray untouched • red weak • orange shaky • green good pass • blue mastered.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }

                ForEach(["Week 15", "Week 16", "Week 17"], id: \.self) { week in
                    TrackerWeekSection(
                        week: week,
                        items: focusItems.filter { $0.week == week },
                        base: base,
                        open: open
                    )
                }
                ...
                GlassCard {
                    VStack(alignment: .leading, spacing: 12) {
                        PanelHeader("Future Framework", "Small group, IMCQ, Esoft, OSPE, and other events are staged but not dominant yet.")

                        let future = items.filter { !["Lecture", "DLA", "PQ"].contains($0.type) }

                        if future.isEmpty {
                            Text("No framework items loaded.")
                                .foregroundStyle(.secondary)
                        } else {
                            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 10), count: 3), spacing: 10) {
                                ForEach(future) { item in
                                    TrackerPill(item: item, compact: true)
                                }
                            }
                        }
                    }
    ...
    func completed(_ group: [CourseTrackerItem]) -> Int {
        group.filter { ["green", "blue"].contains($0.color.lowercased()) || ["pass_1", "pass_2", "mastered"].contains($0.status) }.count
    }
}

struct TrackerWeekSection: View {
    let week: String
    let items: [CourseTrackerItem]
    let base: String
    let open: (String) -> Void
    ...
    var body: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: 16) {
                PanelHeader(week, "\(items.count) focus items")

                ForEach(["Lecture", "DLA", "PQ"], id: \.self) { type in
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            HStack {
                                Text(type)
                                    ...
                                    .foregroundStyle(.secondary)
                                Spacer()
                                Text("\(group.filter { ["green", "blue"].contains($0.color.lowercased()) }.count)/\(group.count)")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                            }
                            ...
                            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 10), count: 4), spacing: 10) {
                                ForEach(group) { item in
                                    TrackerPill(item: item, compact: false)
                                }
                            }
                        }
                    }
            ...
            GeometryReader { geo in
                ZStack(alignment: .leading) {
                    ...
                    RoundedRectangle(cornerRadius: 99)
                        .fill(
                            LinearGradient(
                                colors: [.cyan.opacity(0.75), .blue.opacity(0.75), .purple.opacity(0.65)],
                                startPoint: .leading,
                                endPoint: .trailing
                            )
                        )
...
struct TrackerPill: View {
    ...
    var swatch: Color {
        ...
        case "orange": return .orange.opacity(0.78)
        case "green": return .green.opacity(0.72)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: compact ? 4 : 7) {
            HStack {
                ...
                Circle()
                    ...
                    .shadow(color: swatch.opacity(0.5), radius: 8)
            }
        ...
        .shadow(color: swatch.opacity(0.08), radius: 12)
    }
}
...
struct PromptPage: View {
    ...
    func savePrompt() {
        ...
        append(csv.trimmingCharacters(in: .newlines), to: "\(base)/09 Admin/App Data/prompt_library.csv")
