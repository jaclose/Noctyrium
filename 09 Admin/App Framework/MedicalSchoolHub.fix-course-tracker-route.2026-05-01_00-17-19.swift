...
struct CourseTrackerPage: View {
    ...
    var completedCount: Int {
        visibleItems.filter { ["green", "blue"].contains($0.color.lowercased()) || $0.status == "completed" || $0.status == "mastered" }.count
    }

    var needsWorkCount: Int {
        visibleItems.filter { $0.color.lowercased() == "red" || $0.status == "needs_work" }.count
    }

    var masteredCount: Int {
        visibleItems.filter { $0.color.lowercased() == "blue" || $0.status == "mastered" }.count
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    HStack(alignment: .center) {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text("Term → module → lecture/DLA/PQ pass ledger.")
                        ...
                        ForEach(terms, id: \.self) { term in
                            Button {
                                ...
                                Text(term)
                                    ...
                                    .padding(.vertical, 9)
                                    .background(selectedTerm == term ? .white.opacity(0.22) : .white.opacity(0.08))
                                    .clipShape(RoundedRectangle(cornerRadius: 14))
                            }
                            .buttonStyle(.plain)
                            .glassButton(selectedTerm == term)
                        }
                    }
                        ...
                        ForEach(modulesByTerm[selectedTerm] ?? [], id: \.self) { module in
                            Button {
                                ...
                                Text(module)
                                    ...
                                    .padding(.vertical, 8)
                                    .background(selectedModule == module ? .white.opacity(0.20) : .white.opacity(0.07))
                                    .clipShape(RoundedRectangle(cornerRadius: 13))
                            }
                            .buttonStyle(.plain)
                            .glassButton(selectedModule == module)
                        }
                    }
                }
            }
            ...
            HStack(spacing: 12) {
                TrackerHero(title: "Items", value: "\(visibleItems.count)", symbol: "list.bullet.rectangle")
                ...
                TrackerHero(title: "Practice", value: "\(practice.count)", symbol: "questionmark.bubble")
            }

            if selectedTerm == "T2" && selectedModule == "NB3" {
                ScrollView {
                    VStack(alignment: .leading, spacing: 14) {
                        TrackerCompactSection(
                            title: "Lectures",
                            subtitle: "NB55–NB76 lecture pass ledger",
                            items: lectures,
                            base: base
                        )
                        ...
                        TrackerCompactSection(
                            title: "DLAs",
                            subtitle: "Dedicated learning activities",
                            items: dlas,
                            base: base
                        )

                        TrackerCompactSection(
                            title: "Weekly Practice / PQs",
                            subtitle: "Practice questions at the end of each week",
                            items: pqs,
                            base: base
                        )
                        ...
                        PracticeCapturePanel(
                            base: base,
                            newPracticeTitle: $newPracticeTitle,
                            newPracticeType: $newPracticeType
                        )

                        TrackerCompactSection(
                            title: "PLGs / eSofts / IMCQ / Small Groups",
                            subtitle: "Framework items staged for deeper integration",
                            items: practice.filter { $0.type.lowercased() != "pq" },
                            base: base
                        )
                    }
                    .padding(.bottom, 30)
                ...
                GlassCard {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Text("Tracker shell ready. We will populate this course/module once you provide the overview or schedule.")
                            .font(.system(size: 13, weight: .medium, design: .rounded))
                            .foregroundStyle(.secondary)

                        Text("Current fully populated tracker: T2 → NB3.")
                            .font(.system(size: 12, weight: .semibold, design: .rounded))
...
struct TrackerCompactSection: View {
    let title: String
    let subtitle: String
    let items: [CourseTrackerItem]
    let base: String
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(spacing: 4) {
                    ForEach(items) { item in
                        TrackerLedgerRow(item: item, base: base)
                    }
                }
            }
        }
...
struct TrackerLedgerRow: View {
    ...
    var passes: Int { max(0, min(item.quality, 5)) }
    ...
    var body: some View {
        HStack(spacing: 8) {
            Text(item.week.replacingOccurrences(of: "Week ", with: "W"))
            ...
            HStack(spacing: 3) {
                ForEach(0..<5, id: \.self) { index in
                    Circle()
                        .fill(index < passes ? statusColor.opacity(0.92) : .white.opacity(0.10))
            ...
            Button("+") {
                updateTrackerItem(base: base, id: item.id, qualityDelta: 1, newColor: nil)
            }
            .font(.system(size: 11, weight: .bold))
            .buttonStyle(.plain)
            .glassButton(false, radius: 8)
            .frame(width: 26)
            ...
            Menu {
                Button("Needs Work") {
                    updateTrackerItem(base: base, id: item.id, qualityDelta: 0, newColor: "red")
                }
                Button("Shaky") {
                    updateTrackerItem(base: base, id: item.id, qualityDelta: 0, newColor: "orange")
                }
                Button("Completed") {
                    updateTrackerItem(base: base, id: item.id, qualityDelta: 0, newColor: "green")
                }
                Button("Mastered") {
                    updateTrackerItem(base: base, id: item.id, qualityDelta: 0, newColor: "blue")
                }
                Button("Reset") {
                    updateTrackerItem(base: base, id: item.id, qualityDelta: -999, newColor: "gray")
                }
            } label: {
                Text(statusLabel)
                    .font(.system(size: 10, weight: .bold, design: .rounded))
                    ...
                    .frame(width: 82)
                    .padding(.vertical, 5)
                    .background(statusColor.opacity(0.24))
                    .clipShape(Capsule())
            }
...
struct PracticeCapturePanel: View {
    let base: String
    @Binding var newPracticeTitle: String
    @Binding var newPracticeType: String

    let practiceTypes = ["PLG", "PQ", "eSoft", "IMCQ", "Small Group"]

    var body: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: 10) {
                PanelHeader("Add Practice / PLG / eSoft / IMCQ", "Adds a new row into the tracker CSV")
                ...
                HStack(spacing: 8) {
                    ...
                    TextField("Example: PLG 17 neurocognitive review", text: $newPracticeTitle)
                    ...
                    Button("Add") {
                        let trimmed = newPracticeTitle.trimmingCharacters(in: .whitespacesAndNewlines)
                        guard !trimmed.isEmpty else { return }
                        addTrackerPracticeItem(base: base, type: newPracticeType, title: trimmed)
                        newPracticeTitle = ""
                    }
                    .buttonStyle(.plain)
                    .glassButton()
