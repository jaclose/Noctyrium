...
var body: some View {
    ...
    func startNewStudyDay() {
        ...
        let archiveDir = logDir.appendingPathComponent("Archived Study Logs", isDirectory: true)

        try? fm.createDirectory(at: archiveDir, withIntermediateDirectories: true)

        if fm.fileExists(atPath: logFile) {
            let archiveURL = archiveDir.appendingPathComponent("study_log_before_new_day_\(stamp).csv")
            try? fm.copyItem(at: logURL, to: archiveURL)
        }

        let freshLog = """
date,type,minutes,cards,note
\(isoStamp),System,0,0,Start New Study Day hard reset → \(studyDay)

"""

        try? freshLog.write(toFile: logFile, atomically: true, encoding: .utf8)
