...
struct CourseTrackerPage: View {
    ...
    var currentItems: [CourseTrackerItem] {
        if selectedLeaf == "T2 • NB3" { return items }
        if selectedLeaf == "T2 • BSCE" { return bsceItems }
        return generatedModuleItems(for: selectedLeaf)
    }
    ...
    func generatedModuleItems(for key: String) -> [CourseTrackerItem] {
        guard key.contains("FTM") || key.contains("MSK") || key.contains("CPR") || key.contains("ER") || key.contains("DM") || key.contains("NB1") || key.contains("NB2") else {
            return []
        }

        let clean = key.replacingOccurrences(of: " • ", with: "-")
        let module = key.components(separatedBy: " • ").last ?? key

        return [
            CourseTrackerItem(id: "\(clean)-PLG1", week: module, type: "PLG", label: "\(module) PLG 1", status: "not_started", quality: 0, color: "blue", notes: "", updated: ""),
            CourseTrackerItem(id: "\(clean)-PLG2", week: module, type: "PLG", label: "\(module) PLG 2", status: "not_started", quality: 0, color: "blue", notes: "", updated: ""),
            CourseTrackerItem(id: "\(clean)-ES1", week: module, type: "Esoft", label: "\(module) eSoft 1", status: "not_started", quality: 0, color: "blue", notes: "", updated: ""),
            CourseTrackerItem(id: "\(clean)-ES2", week: module, type: "Esoft", label: "\(module) eSoft 2", status: "not_started", quality: 0, color: "blue", notes: "", updated: ""),
            CourseTrackerItem(id: "\(clean)-ES3", week: module, type: "Esoft", label: "\(module) eSoft 3", status: "not_started", quality: 0, color: "blue", notes: "", updated: ""),
            CourseTrackerItem(id: "\(clean)-IM1", week: module, type: "IMCQ", label: "\(module) IMCQ 1", status: "not_started", quality: 0, color: "blue", notes: "", updated: ""),
            CourseTrackerItem(id: "\(clean)-IM2", week: module, type: "IMCQ", label: "\(module) IMCQ 2", status: "not_started", quality: 0, color: "blue", notes: "", updated: ""),
            CourseTrackerItem(id: "\(clean)-PATH", week: module, type: "Pathways", label: "\(module) pathways / correlates review", status: "not_started", quality: 0, color: "blue", notes: "", updated: ""),
            CourseTrackerItem(id: "\(clean)-TERM", week: module, type: "Term Review", label: "\(module) term-wide review", status: "not_started", quality: 0, color: "blue", notes: "", updated: "")
        ]
    }
}
...
struct TrackerDenseRow: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
                ...
                HStack {
                    ...
                    HStack(spacing: 3) {
                        ForEach(0..<3, id: \.self) { i in
                            Image(systemName: i < item.quality ? "checkmark.circle.fill" : "circle")
                                .font(.system(size: 10, weight: .bold))
                                .foregroundColor(i < item.quality ? mainColor : .white.opacity(0.25))
                        }
                    }
            ...
            HStack(spacing: 4) {
                Button("+") { updater(item, "addPass") }
                Button("−") { updater(item, "removePass") }
                Button("Anki") { updater(item, "ankiPass") }
                Button("Work") { updater(item, "needsWork") }
                Button("Mature") { updater(item, "mastered") }
            }
            .buttonStyle(.plain)
...
func extractAnkiPasses(_ note: String) -> Int {
    let marker = "anki="
    guard let range = note.range(of: marker) else { return 0 }
