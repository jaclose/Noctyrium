...
today_minutes() {
  TODAY=$(date +"%Y-%m-%d")
  awk -F',' -v today="$TODAY" 'NR>1 && substr($1,1,10)==today {sum += $3} END {print sum+0}' "$LOG"
}

today_cards() {
  TODAY=$(date +"%Y-%m-%d")
  awk -F',' -v today="$TODAY" 'NR>1 && substr($1,1,10)==today {sum += $4} END {print sum+0}' "$LOG"
}

manual_minutes_today() {
  TODAY=$(date +"%Y-%m-%d")
  awk -F',' -v today="$TODAY" 'NR>1 && substr($1,1,10)==today && $6!="rejected" {sum += $3} END {print sum+0}' "$MANUAL"
}
