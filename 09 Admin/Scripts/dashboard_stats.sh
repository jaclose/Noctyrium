...
NOCTDATA="$APPDATA/Noctyrium"
PROD="$NOCTDATA/Productivity"
DAYS="$PROD/Days"
...
ACTIVE_DAY_FILE="$PROD/active_study_day.txt"
LEGACY_DAY_FILE="$APPDATA/manual_study_day.txt"

mkdir -p "$APPDATA" "$PROD" "$DAYS"
...
noct_day() {
  local d
  d="$(cat "$ACTIVE_DAY_FILE" 2>/dev/null | tr -d '
 ')"
  if [ -z "$d" ]; then
    d="$(cat "$LEGACY_DAY_FILE" 2>/dev/null | tr -d '
 ')"
  fi
  if [ -z "$d" ]; then
    d="$(date +"%Y-%m-%d")"
...
today_minutes() {
  ...
  f="$(active_daily_file)"
  [ -f "$f" ] || { echo 0; return; }
  awk -F',' 'NR>1 {sum += $3} END {print sum+0}' "$f"
}
...
today_cards() {
  ...
  f="$(active_daily_file)"
  [ -f "$f" ] || { echo 0; return; }
  awk -F',' 'NR>1 {sum += $4} END {print sum+0}' "$f"
}

manual_minutes_today() {
  DAY="$(noct_day)"
  awk -F',' -v day="$DAY" 'NR>1 && substr($1,1,10)==day && $6!="rejected" {sum += $3} END {print sum+0}' "$MANUAL"
}
...
task_done_today() {
  TODAY="$(noct_day)"
  awk -F',' -v today="$TODAY" 'NR>1 && $5=="done" && substr($6,1,10)==today {c++} END {print c+0}' "$TASKS"
}
