...
function StudyWorkflowSettings() {
  ...
  function setKindPasses(kind: "Lecture"|"DLA"|"PQ", lecturePasses: number) { store.updateProfile({ studyWorkflow: { ...workflow, configured: true, itemKindDefaults: { ...workflow.itemKindDefaults, [kind]: { ...workflow.itemKindDefaults?.[kind], lecturePasses } } } }); }
  return <div className="backup-actions-panel"><div><div className="sync-title">How you study</div><div className="sub">Optional learner defaults. Courses and individual items can override these without changing your original choices.</div></div><div className="row wrap gap8" aria-label="Study methods">{STUDY_METHOD_OPTIONS.map((option) => <button key={option.id} type="button" className={`filter-pill ${enabled.has(option.id) ? "on" : ""}`} aria-pressed={enabled.has(option.id)} onClick={() => toggle(option.id)}>{option.label}</button>)}</div><div className="settings-target-grid"><label className="stack gap6"><span>Usual lecture passes</span><input className="field" type="number" min={1} max={6} value={workflow.lecturePasses ?? 2} onChange={(event) => store.updateProfile({ studyWorkflow: { ...workflow, configured: true, lecturePasses: Number(event.target.value) } })}/></label><label className="stack gap6"><span>Review again after (days)</span><input className="field" type="number" min={1} max={14} value={workflow.reviewAfterDays ?? 3} onChange={(event) => store.updateProfile({ studyWorkflow: { ...workflow, configured: true, reviewAfterDays: Number(event.target.value) } })}/></label></div><StudyMethodFollowUps workflow={workflow} onChange={studyWorkflow => store.updateProfile({ studyWorkflow })} /><details><summary>Defaults by item type</summary><p className="sub">Set lightweight pass defaults. Course and item overrides still win.</p><div className="settings-target-grid">{(["Lecture","DLA","PQ"] as const).map(kind=><label className="stack gap6" key={kind}><span>{kind}</span><input className="field" aria-label={`${kind} default passes`} type="number" min={1} max={6} value={workflow.itemKindDefaults?.[kind]?.lecturePasses??workflow.lecturePasses??2} onChange={event=>setKindPasses(kind,Number(event.target.value))}/></label>)}</div></details><label className="stack gap6"><span>Other — tell AXOM how you study</span><textarea className="field" value={workflow.customContext ?? ""} onChange={(event) => store.updateProfile({ studyWorkflow: { ...workflow, configured: true, customContext: event.target.value } })}/></label><div className="sub">AXOM keeps this text exactly as written. Suggestions below come from fixed word rules (not AI), are shown for you to confirm, and are never applied automatically.</div><StudyTextSuggestions workflow={workflow} onApply={studyWorkflow => store.updateProfile({ studyWorkflow })} /></div>;
}
